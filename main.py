...
def main():
  ...
  parser.add_argument('-d', '--delete_after_run',   help='Choose if delete after conversion [y/n], DEFAULT: n', default="n")
  ...
  images = [inputfolder+"/"+image for image in listdir(inputfolder) if ( isfile(join(inputfolder,image)) and (".webp" in image) )]
  ...
  for image in images:
    ...
    with Image.open(image) as im:
      ...
      try:
        im.save(name, ".gif", save_all=True)
        optimize(name)
        if deletion:
          remove(image)
